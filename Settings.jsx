        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between py-3 border-b border-white/5">
            <div>
              ...
              <p className="text-[#A3A3A3] text-sm mt-0.5">Deducted from revenue (not charged to customer)</p>
            </div>
            <button
              type="button"
              onClick={() => setSettings(prev => ({ ...prev, platformFeeEnabled: !prev.platformFeeEnabled }))}
              className="relative w-14 h-7 rounded-full transition-colors"
              style={{ background: settings.platformFeeEnabled ? '#D4AF37' : 'rgba(255,255,255,0.1)' }}
            >
              <span
                className="absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-transform"
                style={{ transform: settings.platformFeeEnabled ? 'translateX(28px)' : 'translateX(4px)' }}
              />
            </button>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-white text-sm font-medium mb-2">
                  {settings.platformFeeType === 'percentage' ? 'Fee Percentage (%)' : 'Fixed Fee Amount'}
                </label>
                <input
                  type="number"
                  min="0"
                  ...
                  className="w-full bg-black/20 border border-white/10 text-white placeholder:text-neutral-600 focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] rounded-sm h-12 px-4 transition-all"
                  placeholder={settings.platformFeeType === 'percentage' ? 'e.g., 2' : 'e.g., 50'}
                />
              </div>
            </div>
          )}
