...
if (typeof document !== 'undefined' && !document.getElementById('om-food-css')) {
  ...
  el.textContent = `
    ...
    .omf-card:hover { border-color: rgba(255,140,0,0.25); }

    /* Table rows */
    .omf-row { border-bottom: 1px solid rgba(255,255,255,0.05); transition: background 150ms; cursor: pointer; }
    .omf-row:hover { background: rgba(255,140,0,0.04); }
    .omf-row-open { background: rgba(255,140,0,0.03) !important; }
    ...
    .omf-btn-orange { background: linear-gradient(135deg,#FF7A20,#E55A00); color:#fff; box-shadow: 0 3px 12px rgba(255,120,0,0.3); }
    .omf-btn-orange:hover { box-shadow: 0 5px 18px rgba(255,120,0,0.45); }
    .omf-btn-ghost  { background: rgba(255,255,255,0.05); color: #7a6a55; border-color: rgba(255,255,255,0.08); }
    ...
    .omf-btn-green  { background: rgba(16,185,129,0.1); color: #34d399; border-color: rgba(16,185,129,0.2); }
    .omf-btn-green:hover  { background: rgba(16,185,129,0.2); }
    ...
    .omf-input:focus { border-color: rgba(255,140,0,0.55); box-shadow: 0 0 0 3px rgba(255,140,0,0.1); }
    .omf-input::placeholder { color: #3d3020; }
    .omf-input[type="date"] { color-scheme: dark; }

    /* Selects */
    .omf-select {
      background: #1c1509; border: 1.5px solid rgba(255,255,255,0.08); border-radius: 9px;
      color: #fff8ee; padding: 6px 10px; font-size: 12px; font-weight: 700;
      font-family: 'Nunito', system-ui, sans-serif;
      outline: none; cursor: pointer; transition: border-color 160ms;
    }
    .omf-select:focus { border-color: rgba(255,140,0,0.5); }
    .omf-select option { background: #1c1509; }
    ...
    .omf-tab-on  { background: linear-gradient(135deg,#FF7A20,#E55A00); color: #fff; box-shadow: 0 3px 14px rgba(255,120,0,0.35); }
    .omf-tab-off { background: rgba(255,255,255,0.04); color: #7a6a55; border-color: rgba(255,255,255,0.07); }
    .omf-tab-off:hover { background: rgba(255,140,0,0.08); color: #FF7A20; border-color: rgba(255,140,0,0.2); }

    /* Scrollbar */
    .omf-scroll::-webkit-scrollbar { width: 4px; height: 4px; }
    .omf-scroll::-webkit-scrollbar-track { background: transparent; }
    .omf-scroll::-webkit-scrollbar-thumb { background: rgba(255,140,0,0.25); border-radius: 4px; }
    ...
    /* Order number style */
    .omf-ordnum { font-family: 'Fredoka One', system-ui, sans-serif; color: #FF7A20; }
    ...
    .omf-sheet-grip { width: 36px; height: 4px; border-radius: 4px; background: rgba(255,140,0,0.28); }
  `;
...
const STATUS = {
  ...
  completed: { emoji: '✅', label: 'Done',       bg: 'rgba(16,185,129,0.12)',  color: '#34d399', bd: 'rgba(16,185,129,0.22)' },
  cancelled: { emoji: '❌', label: 'Cancelled',  bg: 'rgba(220,50,50,0.12)',   color: '#f87171', bd: 'rgba(220,50,50,0.22)' },
};
const PAYMENT = {
  paid:    { emoji: '💰', bg: 'rgba(16,185,129,0.12)',  color: '#34d399', bd: 'rgba(16,185,129,0.22)' },
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{borderBottom:'1px solid rgba(255,140,0,0.12)'}}>
            <div>
              ...
              <p className="text-xs mt-0.5" style={{color:'#7a6a55'}}>#{order.orderNumber ? String(order.orderNumber).padStart(3,'0') : order.id.slice(0,6)}{order.customerName ? ` · ${order.customerName}` : ''}</p>
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center" style={{background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.2)'}}>
          ...
          <div className="flex-1 overflow-y-auto omf-scroll px-4 py-3 space-y-2">
            {loadingMenu ? (
              <div className="flex flex-col items-center justify-center py-12 gap-2">
                ...
                <p className="text-sm" style={{color:'#7a6a55'}}>Loading menu…</p>
              </div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-10"><div className="text-4xl mb-2">🫙</div><p className="text-sm" style={{color:'#4a3f35'}}>No items found</p></div>
            ) : filtered.map(item => {
              ...
              return (
                <div key={item.id} className="flex items-center justify-between p-3 rounded-xl transition-all"
                  ...
                  onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(255,140,0,0.22)';e.currentTarget.style.background='rgba(255,140,0,0.04)';}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.06)';e.currentTarget.style.background='rgba(255,255,255,0.03)';}}
                >
                  <div className="flex-1 min-w-0 mr-3">
                    <p className="text-white text-sm font-bold truncate">{item.name}</p>
                    {item.category && <p className="text-xs mt-0.5" style={{color:'#4a3f35'}}>{item.category}</p>}
                    {hasV ? (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {iv.map((v,vi)=><span key={vi} className="omf-badge" style={{background:'rgba(255,140,0,0.12)',color:'#FF7A20',borderColor:'rgba(255,140,0,0.25)',fontSize:'0.65rem'}}>{v.name||`S${vi+1}`} {CUR}{fmtN(v.price)}</span>)}
                      </div>
                    ) : <p className="text-sm font-black mt-1" style={{color:'#FF7A20'}}>{dp}</p>}
                  </div>
                  {!hasV && qty > 0 ? (
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button onClick={()=>{const e=newCart.find(i=>i.id===item.id);if(e)directAdd({...e});else addToCart(item);}} className="w-7 h-7 rounded-full flex items-center justify-center text-white" style={{background:'linear-gradient(135deg,#FF7A20,#E55A00)'}}><Plus className="w-3 h-3"/></button>
                    </div>
                  ) : (
          ...
          {/* Cart footer */}
          {newCart.length > 0 && (
            <div className="px-4 py-4 flex-shrink-0 space-y-3" style={{borderTop:'1px solid rgba(255,140,0,0.12)',background:'rgba(0,0,0,0.25)'}}>
              <p className="text-xs font-black" style={{color:'#FF7A20'}}>🛒 Cart · {newCart.length} item{newCart.length!==1?'s':''}</p>
              <div className="space-y-1.5">
                {newCart.map((item,idx)=>{
                  ...
                  return <div key={idx} className="flex justify-between text-xs" style={{color:'#7a6a55'}}><span>{item.name}{item.selectedVariant?` (${item.selectedVariant})`:''} ×{item.quantity}{adds.length>0?` +${adds.length} add-on${adds.length>1?'s':''}`:''}</span><span className="text-white font-bold">{CUR}{fmtN(lt)}</span></div>;
                })}
                <div className="flex justify-between text-sm font-black pt-1" style={{borderTop:'1px solid rgba(255,140,0,0.1)'}}>
                  <span style={{color:'#7a6a55'}}>New items total</span>
                  <span style={{color:'#FF7A20'}}>{CUR}{fmtN(cartTotal)}</span>
                </div>
              </div>
              <motion.button whileHover={{scale:1.01}} whileTap={{scale:0.97}} onClick={handleSave} disabled={saving}
                ...
                style={{background:'linear-gradient(135deg,#FF7A20,#E55A00)',boxShadow:'0 6px 20px rgba(255,120,0,0.3)'}}
              >
        ...
        {addonModal && <AddOnModal item={addonModal} onConfirm={entry=>{directAdd(entry);setAddonModal(null);}} onClose={()=>setAddonModal(null)} currencySymbol={CUR} primaryColor="#FF7A20" theme="dark"/>}
      </motion.div>
    </AnimatePresence>
  );
};
...
const OrdersManagement = () => {
  ...
  // ─── Item list renderer (shared desktop+mobile, only data-testid differs) ──
          ...
          return (
            <div key={idx} className="pb-2.5" style={{borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
              <div className="flex justify-between items-start gap-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {item.isFree
                    ? <span className="omf-badge" style={{background:'rgba(16,185,129,0.12)',color:'#34d399',borderColor:'rgba(16,185,129,0.2)'}}>🎁 FREE</span>
                    : <span className="font-black text-sm" style={{color:'#FF7A20'}}>{CUR}{total.toFixed(2)}</span>
                  }
              ...
              <p className="text-xs mt-0.5 ml-4" style={{color:'#555'}}>
                {item.isFree?<span style={{color:'#34d399'}}>🎁 FREE · was {CUR}{(parseFloat(item.actualPrice)||0).toFixed(2)}</span>:<>Base: {CUR}{base.toFixed(2)}{qty>1?` ×${qty}`:''}</>}
              </p>
              {item.comboItems?.length>0&&<div className="ml-4 mt-0.5 space-y-0.5">{item.comboItems.map((ci,ci2)=><p key={ci2} className="text-xs" style={{color:'#4a3f35'}}>🔗 {ci.name}{ci.quantity>1?` ×${ci.quantity}`:''}</p>)}</div>}
              {adds.length>0?(
              ...
              ):<p className="text-xs ml-4 mt-0.5 italic" style={{color:'#4a3f35'}}>No add-ons</p>}
            </div>
          );
        })}
        {/* Totals */}
        <div className="pt-2.5 mt-1 space-y-1.5" style={{borderTop:'1px solid rgba(255,140,0,0.1)'}}>
          <div className="flex justify-between text-xs" style={{color:'#5a4f3f'}}><span>🧮 Items Total</span><span>{CUR}{itemsTotal.toFixed(2)}</span></div>
          {addonsTotal>0&&<div className="flex justify-between text-xs" style={{color:'#5a4f3f'}}><span>✨ Add-ons</span><span>+{CUR}{addonsTotal.toFixed(2)}</span></div>}
          {sN(order?.gstAmount)>0&&<div className="flex justify-between text-xs" style={{color:'#4a3f2f'}}><span>🏛️ GST</span><span>+{CUR}{sN(order.gstAmount).toFixed(2)}</span></div>}
          {sN(order?.taxAmount)>0&&<div className="flex justify-between text-xs" style={{color:'#4a3f2f'}}><span>🏛️ Tax</span><span>+{CUR}{sN(order.taxAmount).toFixed(2)}</span></div>}
          {sN(order?.serviceChargeAmount)>0&&<div className="flex justify-between text-xs" style={{color:'#4a3f2f'}}><span>🛎️ Service Charge</span><span>+{CUR}{sN(order.serviceChargeAmount).toFixed(2)}</span></div>}
          {sN(order?.platformFeeAmount)>0&&<div className="flex justify-between text-xs" style={{color:'#4a3f2f'}}><span>💻 Platform Fee</span><span>+{CUR}{sN(order.platformFeeAmount).toFixed(2)}</span></div>}
          <div className="flex justify-between font-black text-sm pt-1.5" style={{borderTop:'1px solid rgba(255,140,0,0.1)'}}>
            <span className="text-white">💵 Grand Total</span>
            <span style={{color:'#FF7A20'}}>{CUR}{(sub+fees).toFixed(2)}</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="omf space-y-5 relative">
          ...
          <div className="fixed inset-0 z-[200]">
            ...
            <div className="absolute inset-x-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center">
              <div className="relative w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl overflow-hidden omf-sheet" onClick={e=>e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between px-6 py-4" style={{borderBottom:'1px solid rgba(255,140,0,0.12)'}}>
                  <div><h3 className="omf-title text-white font-bold text-base">📏 Select Size</h3><p className="text-xs mt-0.5" style={{color:'#7a6a55'}}>{vItem.name}</p></div>
                  <button onClick={()=>setAddItemsVariantModal(null)} className="w-8 h-8 rounded-full flex items-center justify-center" style={{background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.2)'}}><X className="w-4 h-4" style={{color:'#FF7A20'}}/></button>
                </div>
                <div className="px-5 py-4 space-y-2.5 pb-8">
                  {vars.map((v,vi)=>(
                    <button key={vi} onClick={()=>{setAddItemsVariantModal(null);addItemsVariantAddRef.current?.(vItem,v);}}
                      ...
                      style={{background:'rgba(255,140,0,0.1)',border:'1.5px solid rgba(255,140,0,0.25)',color:'#FF7A20'}}
      ...
      <AnimatePresence>
        {newOrderNotification&&(
          <motion.div initial={{opacity:0,x:100,scale:0.9}} animate={{opacity:1,x:0,scale:1}} exit={{opacity:0,x:100,scale:0.9}}
            ...
            style={{background:'linear-gradient(135deg,#FF7A20,#E55A00)',boxShadow:'0 20px 60px rgba(255,120,0,0.5),0 0 0 1px rgba(255,255,255,0.15)'}}
      ...
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={()=>setSoundEnabled(!soundEnabled)} data-testid="sound-toggle" className="omf-btn"
            style={{padding:'9px 14px',fontSize:'13px',borderRadius:'12px',background:soundEnabled?'rgba(255,140,0,0.12)':'rgba(255,255,255,0.04)',color:soundEnabled?'#FF7A20':'#7a6a55',border:`1.5px solid ${soundEnabled?'rgba(255,140,0,0.25)':'rgba(255,255,255,0.07)'}`}}
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="flex-1">
          <label className="block text-xs font-bold mb-1.5" style={{color:'#7a6a55'}}>📅 Start Date</label>
        ...
        <div className="flex-1">
          <label className="block text-xs font-bold mb-1.5" style={{color:'#7a6a55'}}>📅 End Date</label>
      ...
      {loading?(
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-bold" style={{color:'#7a6a55'}}>Fetching orders from the kitchen…</p>
        </div>
        ...
        <div className="omf-card flex flex-col items-center justify-center py-16 gap-3 text-center">
          ...
          <p className="text-sm" style={{color:'#7a6a55'}}>New orders will fly in real-time 🚀</p>
        </div>
      ):(
        <>
          {/* ── Desktop Table ─────────────────────────────────── */}
          <div className="hidden lg:block omf-card overflow-hidden">
            ...
            >
              <table className="w-full">
                <thead>
                  <tr style={{borderBottom:'1px solid rgba(255,140,0,0.12)',background:'rgba(255,140,0,0.04)'}}>
                    {[['🔖','Order #'],['👤','Customer'],['📞','Phone'],['🍽️','Type'],['🪑','Table'],['📦','Items'],['💵','Total'],['💳','Payment'],['📊','Status'],['⚙️','Actions']].map(([em,h])=>(
                      <th key={h} className="text-left px-4 py-3.5 text-xs font-black uppercase tracking-wider" style={{color:'#FF7A20'}}>{em} {h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <React.Fragment key={order.id}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5 text-sm" style={{color:'#7a6a55'}}>{order.customerPhone||'—'}</td>
                          ...
                          <td className="px-4 py-3.5 text-sm text-white font-medium">
                            {order.orderType==='dine-in'?(order.tableNumber?`🪑 ${order.tableNumber}`:'—'):order.orderType==='delivery'?<span style={{color:'#7a6a55',fontSize:11}}>🛵 Delivery</span>:'—'}
                          </td>
                          <td className="px-4 py-3.5 text-sm font-medium" style={{color:'#7a6a55'}}>🍴 {getItemsCount(order.items)}</td>
                          <td className="px-4 py-3.5"><span className="font-black text-sm" style={{color:'#FF7A20'}}>💵 {CUR}{(order.totalAmount||order.total||0).toFixed(0)}</span></td>
                        ...
                        {expandedOrder===order.id&&(
                          <tr>
                            <td colSpan="10" style={{background:'rgba(255,140,0,0.025)',borderBottom:'1px solid rgba(255,140,0,0.08)'}}>
                              <div className="px-5 py-5 grid grid-cols-2 gap-8">
                                ...
                                <div>
                                  ...
                                  <div className="space-y-2.5 text-sm">
                                    <div className="flex items-center gap-2.5" style={{color:'#7a6a55'}}><span>🕐</span><span>{order.createdAt?.toDate?.().toLocaleString()||'N/A'}</span></div>
                                    {order.customerPhone&&<div className="flex items-center gap-2.5" style={{color:'#7a6a55'}}><span>📞</span><span>{order.customerPhone}</span></div>}
                                    {order.orderType==='delivery'&&order.deliveryAddress&&<div className="flex items-start gap-2.5" style={{color:'#7a6a55'}}><span>📍</span><span>{order.deliveryAddress}</span></div>}
                                    {order?.orderType==='delivery'&&(
                                      <div className="text-sm mt-1">
                                        <strong style={{color:'#FF7A20'}}>🛵 Delivery Address:</strong>
                                        <div className="text-white mt-0.5">{order?.deliveryAddress||'N/A'}</div>
                                      </div>
                                    )}
                                    {order.specialInstructions&&(
                                      <div className="mt-3 p-3 rounded-xl" style={{background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.2)'}}>
                                        <p className="text-xs font-black mb-1" style={{color:'#FF7A20'}}>📝 Special Instructions</p>
                                        <p className="text-white text-sm">{order.specialInstructions}</p>
                                      </div>
                                    )}
                                    <div className="text-xs mt-1" style={{color:'#7a6a55'}}>
            ...
            {filteredOrders.map((order,oi)=>{
              ...
              return (
                <div key={order.id} data-testid={`order-card-${order.id}`}
                  className="omf-in" style={{animationDelay:`${Math.min(oi*35,350)}ms`,animationFillMode:'both',background:'#141008',border:'1.5px solid rgba(255,255,255,0.07)',borderRadius:'16px',overflow:'hidden',transition:'border-color 200ms'}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(255,140,0,0.22)';}}
                ...
                >
                  ...
                  <div className="p-4 flex items-center justify-between cursor-pointer" onClick={()=>setExpandedOrder(expandedOrder===order.id?null:order.id)}>
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-xl flex-shrink-0" style={{background:'rgba(255,140,0,0.1)',border:'1.5px solid rgba(255,140,0,0.2)'}}>
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{color:'#7a6a55'}}>{order.customerPhone||'—'}</p>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1.5">
                      <span className="font-black text-sm" style={{color:'#FF7A20'}}>💵 {CUR}{(order.totalAmount||order.total||0).toFixed(0)}</span>
                  ...
                  <div className="px-4 pb-3.5 flex flex-wrap items-center gap-2" style={{borderTop:'1px solid rgba(255,255,255,0.04)'}}>
                    <span className="text-xs font-medium capitalize" style={{color:'#7a6a55'}}>{orderTypeIcon(order.orderType)} {order.orderType}</span>
                    {order.orderType==='dine-in'&&order.tableNumber&&<span className="text-xs text-white font-bold">· Table {order.tableNumber}</span>}
                    {order.orderType==='delivery'&&order.deliveryAddress&&<span className="text-xs truncate max-w-[160px]" style={{color:'#7a6a55'}}>📍 {order.deliveryAddress}</span>}
                    <span className="text-xs font-medium" style={{color:'#7a6a55'}}>· 🍴 {getItemsCount(order.items)}</span>
                  ...
                  {/* Expanded */}
                  {expandedOrder===order.id&&(
                    <div className="px-4 pb-5 space-y-4" style={{borderTop:'1px solid rgba(255,140,0,0.08)'}}>
                      <div className="pt-4">
                      ...
                      {order.specialInstructions&&(
                        <div className="p-3 rounded-xl" style={{background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.2)'}}>
                          <p className="text-xs font-black mb-1" style={{color:'#FF7A20'}}>📝 Special Instructions</p>
                          <p className="text-white text-sm">{order.specialInstructions}</p>
                        </div>
                      )}
                      ...
                      <div onClick={e=>e.stopPropagation()}>
                        {deleteConfirmId===order.id?(
                          ...
                          <button onClick={()=>setDeleteConfirmId(order.id)} className="omf-btn omf-btn-ghost w-full justify-center py-2.5" style={{borderRadius:12,color:'#4a3f35'}}>
