...
const ActionItem = ({ text, index }) => (
  <motion.div
    ...
    className="flex items-start gap-3 p-3 bg-white/3 rounded-lg hover:bg-white/5 transition-colors"
  >
...
const InsightSkeleton = () => (
  <div className="space-y-4">
    {[1, 2, 3, 4].map(i => (
      <div key={i} className="bg-[#0F0F0F] border border-white/5 rounded-xl p-5 animate-pulse">
...
const LockedState = () => (
  ...
  >
    ...
    <p className="text-[#A3A3A3] text-sm max-w-xs leading-relaxed">
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <select
            ...
            className="bg-black/20 border border-white/10 text-white text-sm rounded-sm px-3 h-9 focus:border-[#D4AF37] transition-all"
          >
            <option value={7}  className="bg-[#0F0F0F]">Last 7 days</option>
            <option value={14} className="bg-[#0F0F0F]">Last 14 days</option>
      ...
      {!loading && !insights && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="bg-[#0F0F0F] border border-white/5 rounded-xl p-12 text-center"
        >
      ...
      {!loading && insights && (
        <div className="space-y-4">
          ...
          {analytics && (
            ...
            >
              {[
                ...
                <div key={i} className="bg-[#0F0F0F] border border-white/5 rounded-xl p-4 text-center">
                  <p className="text-2xl font-black" style={{ color: stat.color }}>{stat.value}</p>
                  <p className="text-[#A3A3A3] text-xs mt-1">{stat.label}</p>
                </div>
              ))}
            </motion.div>
          )}
          ...
          <SectionCard title="Revenue Analysis" icon={TrendingUp} delay={1} accent="#10B981">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-white/3 rounded-lg p-3">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-1">Trend</p>
                <p className="text-white font-semibold capitalize">{insights.revenue?.trend}</p>
              </div>
              <div className="bg-white/3 rounded-lg p-3">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-1">Profit Est.</p>
                <p className="text-white font-semibold text-sm">{insights.revenue?.profitEstimation}</p>
              </div>
              <div className="bg-white/3 rounded-lg p-3">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-1">Next Week Target</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <SectionCard title="Star Performers" icon={Star} delay={2} accent="#D4AF37">
              ...
              {(insights.products?.comboSuggestions || []).length > 0 && (
                <>
                  <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mt-4 mb-2">Combo Ideas</p>
            ...
            <SectionCard title="Product Strategy" icon={Target} delay={3} accent="#3B82F6">
              {(insights.products?.pricingOpportunities || []).length > 0 && (
                <>
                  <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-2">Pricing Opportunities</p>
              ...
              {(insights.products?.toPromote || []).length > 0 && (
                <>
                  <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mt-4 mb-2">Push These Items</p>
          ...
          <SectionCard title="Operations & Staffing" icon={Clock} delay={4} accent="#8B5CF6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white/3 rounded-lg p-4">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-2">⚡ Peak Hours</p>
                <p className="text-[#D1D1D1] text-sm leading-relaxed">{insights.operations?.peakHourStrategy}</p>
              </div>
              <div className="bg-white/3 rounded-lg p-4">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-2">😴 Off-Peak</p>
                <p className="text-[#D1D1D1] text-sm leading-relaxed">{insights.operations?.offPeakStrategy}</p>
              </div>
              <div className="bg-white/3 rounded-lg p-4">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-2">📦 Inventory</p>
                <p className="text-[#D1D1D1] text-sm leading-relaxed">{insights.operations?.inventoryAlert}</p>
              </div>
              <div className="bg-white/3 rounded-lg p-4">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-2">👥 Staffing</p>
          ...
          <SectionCard title="Growth Strategies" icon={Zap} delay={5} accent="#F59E0B">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-3">Upsell Tactics</p>
                {(insights.growth?.upsellStrategies || []).map((s, i) => (
                  <InsightBullet key={i} text={s} color="#F59E0B" />
                ))}
              </div>
              <div>
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-3">Time-Based Offers</p>
            ...
            {insights.growth?.externalPlatformStrategy && (
              <div className="mt-4 p-4 bg-orange-500/5 border border-orange-500/20 rounded-lg">
                <p className="text-[#A3A3A3] text-xs uppercase tracking-wide mb-1">🟠 Zomato / Swiggy</p>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#555]">{insights.marketing?.topOffer}</span>
                </div>
                {expanded.marketing ? <ChevronUp className="w-4 h-4 text-[#A3A3A3]" /> : <ChevronDown className="w-4 h-4 text-[#A3A3A3]" />}
              </div>
              <AnimatePresence>
                {expanded.marketing && (
                  <motion.div
                  ...
                  >
                    <pre className="text-[#D1D1D1] text-sm leading-relaxed whitespace-pre-wrap font-sans bg-white/3 rounded-lg p-4 border border-white/5">
                    ...
                    <button
                      ...
                      className="mt-3 flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-sm text-sm transition-all"
                    >
                      📋 Copy Message
                    </button>
                  </motion.div>
