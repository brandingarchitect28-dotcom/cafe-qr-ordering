...
function injectStaffCSS(isLight) {
  if (typeof document === 'undefined') return;
  const existing = document.getElementById('staff-omf-css');
  ...
  el.textContent = `
    ...
    .stf-tab-off { background: ${C.tabOffBg}; color: ${C.tabOffColor}; border-color: ${C.tabOffBorder}; }
    .stf-tab-off:hover { background: ${C.tabOffHBg}; color: ${C.tabOffHColor}; border-color: ${C.tabOffHBorder}; }

    .stf-tab-bar {
      display: flex; gap: 4px; padding: 4px;
      background: ${C.tabBarBg};
      border: 1.5px solid ${C.tabBarBorder};
      border-radius: 14px; width: fit-content;
    }

    .stf-locked {
      background: ${C.lockedBg};
      border: 1.5px solid ${C.lockedBorder};
      border-radius: 16px;
    }
...
const StaffManagement = () => {
  ...
  useEffect(() => { injectStaffCSS(isLight); }, [isLight]);
  ...
  if (!isEnabled) {
    return (
      <div className="stf-wrap stf-locked flex flex-col items-center justify-center py-24 text-center">
        ...
        <h2 className="stf-title font-black text-2xl mb-2" style={{ color: text }}>
          Staff Management
        </h2>
        <p style={{ color: muted, fontSize: 13, maxWidth: 280 }}>
  ...
  return (
    <div className="stf-wrap space-y-5">
      ...
      <div className="flex items-center justify-between stf-in">
        <div>
          <h2 className="stf-title font-black text-2xl" style={{ color: text }}>Staff Management</h2>
      ...
      <div className="stf-tab-bar stf-in" style={{ animationDelay: '40ms', animationFillMode: 'both' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`stf-tab ${tab === t.id ? 'stf-tab-on' : 'stf-tab-off'}`}>
