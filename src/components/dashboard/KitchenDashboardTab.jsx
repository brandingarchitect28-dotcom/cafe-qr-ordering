...
function injectKitchenCSS(isLight) {
  if (typeof document === 'undefined') return;
  const existing = document.getElementById('kdt-cafe-css');
  if (existing) existing.remove();

  const C = isLight ? {
    cardBg:          '#FFFFFF',
    cardBorder:      'rgba(0,0,0,0.10)',
    cardHover:       'rgba(201,162,39,0.25)',
    orderCardBg:     '#FFFFFF',
    orderCardBorder: 'rgba(0,0,0,0.10)',
    orderCardHover:  'rgba(201,162,39,0.25)',
  } : {
    cardBg:          '#141008',
    cardBorder:      'rgba(255,255,255,0.07)',
    cardHover:       'rgba(201,162,39,0.22)',
    orderCardBg:     '#141008',
    orderCardBorder: 'rgba(255,255,255,0.07)',
    orderCardHover:  'rgba(201,162,39,0.22)',
  };
  ...
  el.textContent = `
    ...
    .kdt-card:hover { border-color: ${C.cardHover}; }
    .kdt-order-card {
      background: ${C.orderCardBg};
      border: 1.5px solid ${C.orderCardBorder};
      border-radius: 14px;
      overflow: hidden;
      transition: border-color 200ms;
    }
    .kdt-order-card:hover { border-color: ${C.orderCardHover}; }
...
const KitchenDashboardTab = () => {
  ...
  useEffect(() => { injectKitchenCSS(isLight); }, [isLight]);
  ...
  const itemTxt    = isLight ? '#333333' : '#7a6a55';
  const tblBadgeBg = isLight ? 'rgba(0,0,0,0.06)'  : 'rgba(255,255,255,0.05)';
  const tblBadgeTxt= isLight ? '#333333'             : '#A3A3A3';
  const tblBadgeBd = isLight ? 'rgba(0,0,0,0.10)'   : 'rgba(255,255,255,0.08)';
  // ─────────────────────────────────────────────────────────────────────────
  ...
  if (loading) return (
    <div className="kdt flex flex-col items-center justify-center py-20 gap-3">
      ...
      <p className="text-sm font-bold" style={{ color: muted }}>Loading kitchen…</p>
    </div>
  );
    ...
    <div className="kdt space-y-5">
      ...
      <div className="kdt-card p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-black kdt-title" style={{ color: text }}>Kitchen Display System</h3>
      ...
      <div className="kdt-card px-4 py-3 flex items-center gap-3">
        <span className="text-xs font-bold flex-shrink-0 flex items-center gap-1" style={{ color: muted }}>
          <Link2 className="w-3 h-3" /> Kitchen URL:
        </span>
        ...
        <button
          ...
          style={{ background: 'rgba(201,162,39,0.1)', color: '#C9A227', border: '1.5px solid rgba(201,162,39,0.2)' }}>
          Copy
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {columns.map(col => (
          <div key={col.id} className="kdt-card p-4 flex items-center gap-3"
            ...
            <div>
              ...
              <p className="text-xs font-bold uppercase tracking-wide" style={{ color: muted }}>{col.label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {activeOrders.length === 0 ? (
        <div className="kdt-card p-12 text-center">
          <Package className="w-12 h-12 mb-3 mx-auto" style={{ color: vfaint }} />
          <p className="font-bold" style={{ color: muted }}>No active orders in the kitchen right now</p>
          <p className="text-xs mt-1 flex items-center justify-center gap-1" style={{ color: faint }}>
            Orders will appear here in real-time <Sparkles className="w-3 h-3" />
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          <p className="text-xs font-black uppercase tracking-widest flex items-center gap-1" style={{ color: '#C9A227' }}>
            <Utensils className="w-3.5 h-3.5" /> Active Orders — {activeOrders.length} total
          </p>
                  ...
                  <div className="flex items-start gap-4 px-4 py-3 flex-wrap">
                    ...
                    {order.tableNumber && (
                      <span className="text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1"
                        style={{ background: tblBadgeBg, color: tblBadgeTxt, border: `1px solid ${tblBadgeBd}` }}>
                        <Armchair className="w-3 h-3" /> Table {order.tableNumber}
                      </span>
                    )}

                    {/* Items */}
                    <div className="flex-1 text-sm min-w-0" style={{ color: itemTxt }}>
                      {order.items?.map((item, idx) => (
                        <div key={idx} className="mb-0.5">
                          ...
                          {item.comboItems?.length > 0 && item.comboItems.map((ci, cIdx) => (
                            <span key={cIdx} className="block text-xs opacity-70 ml-3">
                              — {ci.name}{ci.quantity > 1 ? ` ×${ci.quantity}` : ''}
                            </span>
                          ))}
                          {item.addons?.length > 0 && (
                            <span className="block text-xs opacity-70 ml-3 flex items-center gap-1">
                              <Sparkles className="w-3 h-3" /> {item.addons.map(a => a.name).join(', ')}
                            </span>
                          )}
                        </div>
                      ))}
                    ...
                    {/* Elapsed */}
                    <div className="flex items-center gap-1 text-xs flex-shrink-0 font-bold" style={{ color: faint }}>
