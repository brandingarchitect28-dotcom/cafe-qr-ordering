 ...
 * Shows today's attendance, monthly calendar, and QR scanner.
 */
...
import {
  ...
  ChevronLeft, ChevronRight, Camera, X, Scan,
} from 'lucide-react';
import {
...
// ─── QR Scanner Modal ─────────────────────────────────────────────────────────
// Uses device camera via getUserMedia + canvas frame analysis
...
const QRScannerModal = ({ cafeId, staffList, onClose }) => {
  ...
  // Use BarcodeDetector API if available, else show manual fallback
  useEffect(() => {
  ...
  const handleQRValue = useCallback(async (value) => {
    ...
    setTimeout(() => {
      setStatus('scanning');
      setMessage('Point camera at staff QR code');
      setLastScanned(null);
    }, 3000);
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
      ...
      <motion.div className="relative bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden w-full max-w-sm"
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-48 h-48 relative">
              {['top-left','top-right','bottom-left','bottom-right'].map(corner => (
                <div key={corner} className={`absolute w-8 h-8 border-[#D4AF37] border-2
                  ...
                  ${corner === 'bottom-right' ? 'border-l-0 border-t-0' : ''}
                `} style={{
                  borderTopRightRadius:    corner === 'top-left'    ? 0 : undefined,
                  borderTopLeftRadius:     corner === 'top-right'   ? 0 : undefined,
                  borderBottomLeftRadius:  corner === 'bottom-right'? 0 : undefined,
                  borderBottomRightRadius: corner === 'bottom-left' ? 0 : undefined,
                }} />
              ))}
              {status === 'scanning' && (
                <motion.div className="absolute left-0 right-0 h-0.5 bg-[#D4AF37]/70"
                  animate={{ top: ['0%', '100%', '0%'] }}
        ...
        <div className={`p-4 text-center transition-colors ${
          ...
          {!('BarcodeDetector' in window) && status === 'scanning' && (
            <p className="text-[#555] text-xs mt-1">QR scanning not supported in this browser — use manual check-in below</p>
          )}
        </div>
      </motion.div>
    </motion.div>
    ...
    <div className="bg-[#0F0F0F] border border-white/5 rounded-2xl p-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        <select value={selectedStaff} onChange={e => setSelectedStaff(e.target.value)}
          ...
          {staffList.map(s => <option key={s.id} value={s.id} className="bg-[#0F0F0F]">{s.name}</option>)}
        </select>
      </div>
        ...
        {[...Array(days)].map((_, i) => {
          ...
          return (
            <div key={day}
              ...
              style={{
                background: s ? s.bg : isToday ? 'rgba(212,175,55,0.1)' : 'transparent',
                color:      s ? s.color : isToday ? '#D4AF37' : '#555',
...
const AttendanceDashboard = ({ cafeId }) => {
  ...
  const presentIds = new Set(todayRecs.map(r => r.staffId));
  ...
  return (
    <>
      <AnimatePresence>
        {showScanner && (
          <QRScannerModal cafeId={cafeId} staffList={staffList} onClose={() => setShowScanner(false)} />
        )}
      </AnimatePresence>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-[#0F0F0F] border border-white/5 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/5">
            <p className="text-white font-semibold text-sm">Today — {new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
          </div>
          <div className="divide-y divide-white/5">
            {staffList.map(s => {
              const rec = todayRecs.find(r => r.staffId === s.id);
              const st  = rec ? STATUS[rec.status] : STATUS.absent;
              return (
            ...
            {staffList.length === 0 && (
              <div className="px-5 py-8 text-center text-[#555] text-sm">No staff added yet</div>
            )}
          </div>
        </div>

        {/* Monthly calendar */}
        {staffList.length > 0 && <MonthCalendar cafeId={cafeId} staffList={staffList} />}
      </div>
    </>
  );
};
