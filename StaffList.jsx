/**
 * StaffList.jsx
 * Original working version — restored exactly.
 * Only addition: "Attendance" button per staff card opens AttendanceCalendar modal.
 * All existing logic (showForm, StaffFormModal, QRModal, CRUD) is untouched.
 */
...
import {
  ...
  Phone, CreditCard, Clock, RefreshCw, User, Calendar,
} from 'lucide-react';
...
const inputCls = 'w-full bg-black/20 border border-white/10 text-white placeholder:text-neutral-600 ' +
  'focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] rounded-lg h-11 px-4 text-sm outline-none transition-all';
const labelCls = 'block text-[#A3A3A3] text-xs font-semibold uppercase tracking-wide mb-1.5';

// ─── Staff Form Modal (original — untouched) ──────────────────────────────────
const StaffFormModal = ({ initial, onSave, onClose, saving }) => {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <motion.div className="relative bg-[#0F0F0F] border border-white/10 rounded-2xl w-full max-w-lg p-6 space-y-5 overflow-y-auto max-h-[90vh]"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input className={inputCls} value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Rahul Kumar" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className={inputCls} value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="10-digit number" type="tel" />
            </div>
            <div>
            ...
            <div>
              ...
              <select className={inputCls + ' cursor-pointer'} value={form.salaryType} onChange={e => set('salaryType', e.target.value)}>
                {SALARY_TYPES.map(t => <option key={t} value={t} className="bg-[#0F0F0F]">{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
              </select>
            </div>
            <div>
              <label className={labelCls}>Salary Amount (₹) *</label>
              <input className={inputCls} type="number" min="0" value={form.salaryAmount}
                onChange={e => set('salaryAmount', e.target.value)} placeholder="e.g. 15000" />
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? <><RefreshCw className="w-4 h-4 animate-spin" />Saving…</> : <><Save className="w-4 h-4" />{initial?.id ? 'Update' : 'Add Staff'}</>}
            </button>
          </div>
        </form>
      </motion.div>
...
const QRModal = ({ staff, cafeId, onClose }) => {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={onClose} />
      <motion.div className="relative bg-[#0F0F0F] border border-white/10 rounded-2xl p-8 text-center space-y-5"
        initial={{ scale: 0.9 }} animate={{ scale: 1 }}>
        <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-lg hover:bg-white/10 text-[#A3A3A3]">
          <X className="w-4 h-4" />
        </button>
        <div>
          <h3 className="text-white font-bold text-xl" style={{ fontFamily: 'Playfair Display, serif' }}>{staff.name}</h3>
...
// ─── Role colours (original — untouched) ─────────────────────────────────────
const ROLE_COLORS = {
...
const StaffList = ({ cafeId }) => {
  ...
  // Real-time listener (original — untouched)
  useEffect(() => {
    if (!cafeId) return;
  ...
  return (
    <>
      ...
      <AnimatePresence>
        ...
        {qrTarget && <QRModal staff={qrTarget} cafeId={cafeId} onClose={() => setQrTarget(null)} />}
            ...
            {staff.map((s, i) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-base"
                      style={{ background: `${ROLE_COLORS[s.role] || '#A3A3A3'}18`, color: ROLE_COLORS[s.role] || '#A3A3A3' }}>
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setQrTarget(s)} className="p-2 rounded-lg hover:bg-white/10 text-[#555] hover:text-[#D4AF37] transition-all" title="View QR">
                      <QrCode className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setEditTarget(s)} className="p-2 rounded-lg hover:bg-white/10 text-[#555] hover:text-white transition-all">
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setDeleteId(s.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-[#555] hover:text-red-400 transition-all">
                ...
                <div className="space-y-1.5 text-xs text-[#555]">
                  ...
                  {s.upiId && <div className="flex items-center gap-1.5"><CreditCard className="w-3 h-3" />{s.upiId}</div>}
                  <div className="flex items-center gap-1.5"><Clock className="w-3 h-3" />Shift: {s.shiftStart}</div>
                </div>

                {/* Salary (original) */}
                <div className="mt-3 pt-3 border-t border-white/5 flex justify-between items-center">
                  <span className="text-[#555] text-xs capitalize">{s.salaryType} salary</span>
                  <span className="text-[#D4AF37] font-black text-sm">₹{s.salaryAmount?.toLocaleString('en-IN')}</span>
                </div>

                {/* NEW: Attendance button */}
                <button
                  onClick={() => setCalendarStaff(s)}
                  className="mt-3 w-full flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all"
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setDeleteId(null)}
                          className="flex-1 py-1.5 border border-white/10 text-[#A3A3A3] rounded-lg text-xs">Cancel</button>
                        <button onClick={() => handleDelete(s.id)}
                          className="flex-1 py-1.5 bg-red-500 text-white rounded-lg text-xs font-bold">Remove</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
